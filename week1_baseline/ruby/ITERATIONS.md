...
We want to build a basedline agent that has all the common compnents for building any kind of agent. Things it should include:
- simple agentic loop
...
- it should have global binary execution so we can interact with the CLI
...
It will have poor perception since it doesn't have a way of managing memory, or decision making. It also won't be token effective.

## Technical Design Considerations

- We will use REST APIs directly, so that we can understand how simple it is to interact with managed APIs and how much they vary.
- Some SDKs (even official ones) do not expose all features and REST API will give full access to feature sets.
...
- We should attempt to use the Standard Library (STDs) and avoid introducing third party libraries.

## What should we not use?
- We should avoid using Agent SDKs since they already implement features we are implementing from scratch. They also might limit our ability to implement exactly what we need. (e.g. don't use OpenRouter, Amazon Strands or CoreAgent or LangChain).
...
- You can follow along and make the ruby changes.
- You can treat the ruby implementation as your main implementation.
- If you have no interest in the Python porting, you can ignore those videos.
- You can watch all the videos and then do a single port of the last ruby iteration to the language of your choice.
...
> So we mentioned earlier we need to normalize the repsonses in the prompt backend and so it occurs here I believe we implement that nomralization within the prompt builder and their backends.
...
### 7 The Run DSL (Domain-Specific Language)
...
> The MCP implementation is a 2 hour video and its worth watching but not doing, so I would recommend copying over the MudManager and the 10_standard_tool_library from omenking repo.
