...
These are things we observerd but do not want to fix since future steps will break.
- We have default prompt e.g. prompts/system.md which is supposed to be scoped on task e.g. prompts/<task>.system.md
- Our settings file should accept yml or yaml file types, now it only takes yaml